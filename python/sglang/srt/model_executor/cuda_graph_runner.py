...
class CudaGraphRunner:
    ...
    def can_run(self, forward_batch: ForwardBatch):
        ...
        if self.enable_dp_attention:
            ...
            graph_handle = (max_num_tokens, use_cached_mask, num_stage_cached)
            is_bs_supported = forward_batch.can_run_dp_cuda_graph and (
                (min_num_tokens == max_num_tokens and graph_handle in self.graphs)
                if self.disable_padding
                else max_num_tokens <= self.max_bs
            )
        else:
            graph_handle = (forward_batch.batch_size, use_cached_mask, num_stage_cached)
            is_bs_supported = (
                graph_handle in self.graphs
                if self.disable_padding
    ...
    def capture(self):
        with graph_capture() as graph_capture_context:
            if self.model_runner.hip_attention_config is not None:
            ...
            # FIXME: handle when hip attention is disabled
